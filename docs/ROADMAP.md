...
| Default styles derived from `Theme::DARK` instead of a hardcoded per-widget palette | Raised during `TextInput` review (#608) | Every widget here (`List`, `Button`, `Gauge`, `TextInput`, ...) currently hardcodes its own `new()` default and only applies a `Theme` via an explicit `.theme(...)` call; whether `new()` should instead default to `Theme::DARK` (or take a `Theme` up front) is an open, crate-wide design question, not something to resolve widget-by-widget. |
