...
| Text-input / line-editor widget                                                                  | ftxui (`Input`), ratatui ecosystem (`tui-textarea`) | Scope question first: does retroglyph want to own interactive text entry? Not a clear miss given the free-function/no-retained-tree widget philosophy. A single-line-only prototype (`TextInputState`/`TextInput` in `crates/widgets`, char-index cursor, no selection/undo/IME) is up as a draft PR for #150 to make the tradeoffs concrete; still deferred pending review before anything wider (multi-line, selection, undo/redo) is considered. |
