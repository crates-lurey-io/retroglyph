...
traversal, validation, submission, or layout, and no IME/text composition or multi-line editing
(both stay explicit non-goals). The caret renders as an inverted cell rather than driving a
backend's `Cursor` facet, since `StatefulWidget::render` only has a `Surface`, not a `Backend`.
...
| Semantic widget labels (`accessible_label`/role hook on `Widget`/`InteractiveWidget`, egui/AccessKit-style) for plain mode and future screen-reader output | egui (AccessKit)                  | #649. A crate-wide trait-signature decision that's expensive to change once third-party widgets depend on it, with no concrete consumer driving it yet (unlike #608's text input, which had `dreadmire`). Would also be the natural place to expose `List` selection state that's currently style-only (`highlight_symbol`). Revisit if an AccessKit integration or a non-visual-output consumer materializes. |
