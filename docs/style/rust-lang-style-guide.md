...
1. **Trailing commas are mandatory in multi-line lists** but forbidden in single-line lists. The

   only exception is single-element tuples `(x,)` which need the comma for disambiguation.

1. **Expression-oriented style is explicitly encouraged.** The guide recommends

   `let x = if y { 1 } else { 0 };` over mutable assignment in branches.

1. **Reserved word collisions**: prefer raw identifiers (`r#type`) or trailing underscore (`type_`)

   over misspelling. The guide explicitly calls out `krate` as wrong.

1. **80-char comment width** is narrower than the 100-char code width. Comment-only lines have a

   stricter limit to improve readability.

1. **"Small" is intentionally vague.** The guide delegates the definition of "small" (for

   single-line formatting decisions) to individual tools, recognizing no single threshold works
   everywhere.

1. **Dereferencing over referencing in expressions.** When both `*t op u` and `t op &u` work, prefer

   the dereference form.

1. **Parentheses are encouraged for clarity** in binary expressions, even when not required by

   precedence. Tools should not auto-insert or auto-remove them.

1. **Match arms never start with `|`.** The first pattern in a match arm is bare; `|` appears only
