...
1. **`sdl3` crate (v0.x) is a fork of rust-sdl2, still in migration.** The SDL3 C API itself
...
1. **SDL3 new features relevant to grid rendering:** `SDL_RenderTextureTiled` for tiling a texture
...
1. **Recommendation:** Target SDL2 (`sdl2` crate) for now. It is battle-tested, well-documented, and
...
1. **SDL_Renderer is a hardware-accelerated 2D rendering abstraction.** It sits on top of OpenGL,
...
1. **Canvas<Window> is the primary Rust interface.** You call `window.into_canvas().build()` to get
...
1. **TextureCreator manages GPU texture lifetime.** Created from a Canvas, it ensures textures
...
1. **SDL3 adds `SDL_RenderTextureTiled`** which tiles a source rectangle across a destination area.
...
1. **Texture atlas pattern for cell grids:** Pre-render all needed glyphs into a single large
...
1. **SDL_Renderer batching performance benchmarks** for high cell counts (10,000+) could not be
...
1. **sdl3 crate timeline** for production readiness is uncertain. The migration checklist is public
...
1. **Console platform (Switch/PS/Xbox) SDL usage from Rust** is undocumented publicly due to NDA
...
1. **SDL3 GPU API Rust bindings** (`SDL_shadercross`, compute, custom shaders) are explicitly marked
