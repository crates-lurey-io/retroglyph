...
1. **Key ioctls** -- Two critical ioctl structs control the framebuffer:
...
1. **Pixel formats** -- The `VarScreeninfo` structure defines pixel format via `Bitfield` structs
...
1. **Double buffering** -- Achieved via virtual resolution. Set `yres_virtual = 2 * yres`, then
...
1. **KD_GRAPHICS mode** -- When rendering directly to the framebuffer, you should set the console to
...
1. **DRM/KMS architecture** -- The Direct Rendering Manager is a kernel subsystem for userspace GPU
...
1. **drm-rs crate (Smithay)** -- Safe Rust bindings for the DRM subsystem. Requires implementing
...
1. **Dumb buffers** -- The simplest buffer type, always available on any DRM driver. Created via
...
1. **Double-buffer in userspace** -- Maintain a `Vec<u32>` (or `Vec<u8>`) as a shadow buffer. Render

   all changes there (with dirty tracking), then blit to the dumb buffer/framebuffer in one pass.

1. **Use fontdue for glyph rasterization** -- `no_std` compatible, fast, minimal. Cache all glyphs

   at initialization.

1. **Use evdev for input** -- Enumerate `/dev/input/event*`, select keyboard devices, run an event

   loop on a dedicated thread or async task.

1. **Set KD_GRAPHICS on the console TTY** -- Suppress kernel text output. Restore on exit (critical:

   install signal handlers for SIGTERM, SIGINT, etc.).

1. **Page flip for vsync** -- If using DRM, use atomic commits with page flip events. If using
...
1. **Multi-monitor with DRM** -- The research covers single-output scenarios. Handling multiple

   CRTCs/connectors for multi-monitor setups in a terminal context needs further investigation.

1. **GPU-accelerated 2D via DRM** -- Using GBM buffers + OpenGL ES for hardware-accelerated glyph

   rendering is possible but adds significant complexity. Worth exploring if CPU rendering proves
   too slow at 4K resolutions.

1. **Wayland-less compositing** -- If multiple overlapping widgets need compositing (popups, menus),
...
1. **Color management** -- Framebuffer color profiles, gamma correction, and HDR support on Linux

   are handled differently than under a display server. Not researched here.

1. **Touch input** -- The evdev crate supports absolute axis events for touchscreens, but the actual
