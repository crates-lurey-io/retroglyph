...
2. **Clean, ergonomic API.** `BTermBuilder::simple80x50().with_title("Game").build()` gets you a
...
3. **Multi-backend portability.** Same code runs on OpenGL, WebGPU (Vulkan/Metal), WebGL (WASM),
...
4. **Layered console system.** Multiple consoles with independent grid sizes, tilesets, and
...
5. **Bundled roguelike algorithms.** A\*, Dijkstra maps, field-of-view, Bresenham lines, noise
...
6. **Modular crate design.** Use just `bracket-pathfinding` without the terminal, or just
   `bracket-noise`. The crates are independently useful.
   [Source](https://crates.io/crates/bracket-lib)

7. **DrawBatch system.** Batched rendering with z-order sorting. Collect draw commands and submit
...
2. **Broken on modern platforms.** Multiple open issues report crashes on Wayland (mouse hover
...
3. **Alpha transparency is broken.** Black pixels with alpha get culled instead of rendered. Issue
...
4. **Sprite system is immature.** The author describes it as "in its infancy." SpriteConsole exists
...
5. **Owns the main loop.** `main_loop(context, state)` takes control. No way to drive rendering from
...
6. **Sparse documentation beyond the tutorial.** The usage manual is incomplete ("early work has
...
7. **High CPU usage reported.** The "State of Game Dev in Rust 2024" article notes 97% CPU to render
...
8. **Crossterm backend panics.** Index out of bounds when using tile-based consoles with the
...
9. **Curses backend broken.** Compilation error on `fitscreen` field (issue #329, open since Jan
