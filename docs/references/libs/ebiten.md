...
1. **Automatic draw batching.** For tile-based rendering, drawing hundreds or thousands of SubImages
...
1. **Cross-platform from one codebase.** Desktop + WASM + mobile + consoles. WASM support is

   particularly notable for roguelikes (playable in browser). No code changes needed, just
   `GOOS=js GOARCH=wasm go build`.

1. **Pure Go on desktop.** No C dependencies on desktop/WASM builds (uses purego for system calls).

   Simplifies builds and CI.

1. **Custom shaders (Kage).** Go-like syntax compiles to GLSL/HLSL/Metal. Useful for

   post-processing: CRT effects, fog of war overlays, lighting.
   [Kage tutorial](https://github.com/tinne26/kage-desk)

1. **Active ecosystem.** 13K stars, active Discord, multiple ECS frameworks (donburi, ark), GUI
...
1. **Stable and mature.** Actively maintained since 2013. v2 API is stable. Apache-2.0 licensed.

1. **Automatic texture atlas.** Small images are packed into shared atlas textures (4096x4096),
...
1. **No built-in ECS or scene management.** Ebiten gives you a blank canvas. You need to build or

   import your own ECS (donburi, ark), scene manager (stagehand), input mapper, etc. This is by
   design but means more upfront wiring.

1. **Vector path rendering is slow.** Drawing many vector paths per frame (e.g., hundreds of filled
...
1. **No built-in text console/grid abstraction.** Unlike BearLibTerminal or libtcod, there's no "put
...
1. **Kage shader limitations.** Fragment shaders only (no vertex or compute shaders). The Go-like
...
1. **Context loss handling adds complexity.** Ebitengine records draw commands to restore state
...
1. **Image.At() and ReplacePixels are slow.** Reading pixels back from GPU or bulk-replacing pixels

   causes a pipeline flush. Fine for occasional use, but not for per-frame pixel manipulation.

1. **Not designed for general-purpose GUI.** The maintainer explicitly states it's not oriented
