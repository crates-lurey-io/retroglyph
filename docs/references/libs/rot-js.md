...
1. **The Royal Wedding** - Described as "the most beautiful roguelike" by one developer; cited as

   what drew people to rot.js. [RogueBasin](http://roguebasin.com/index.php/The_Royal_Wedding)

### 7DRL and Other Games

1. **No'hanz** - A turn-based roguelike about traps (polymorph, duplicate, wrath). 7DRL 2019, rated

   4.6/5. [itch.io](https://st33d.itch.io/nohanz)

1. **Blackfeather** - Fantasy dungeon crawler, 7DRL 2015.

   [GitHub](https://github.com/Starstew/Blackfeather)

1. **Lirael's Library** - TypeScript 7DRL 2022. [GitHub](https://github.com/JanLopata/lirael-7drl)
1. **Copy Frogue**, **Lyon's Den**, **Sleeping Beauty**, **Goldfish**, **FunhouseRL**, **RailRL**,
...
1. **Modeled after libtcod** - Familiar API for anyone coming from the traditional roguelike

   development community. The cell-based `draw(x, y, ch, fg, bg)` pattern is intuitive for
   grid-based games.

1. **Multiple display backends** - Text, tiles (Canvas 2D and WebGL), hex, and terminal output, all

   behind the same API. Switching from ASCII to graphical tiles requires only changing options, not
   rewriting rendering code.

1. **Interactive manual** - The [interactive manual](https://ondras.github.io/rot.js/manual/) has
...
1. **Hex support throughout** - Hex grids work with the display, FOV, pathfinding, and map

   generation. Not an afterthought.

1. **Small and focused** - No external dependencies. The minified bundle is small. Does one thing

   (roguelike infrastructure) well.

1. **Browser-native** - Just a `<canvas>` element. No WebGL required for the default backend. Works

   in any modern browser.

1. **Proven track record** - Used in hundreds of 7DRL entries and notable games like Untrusted.
...
1. **FOV inconsistencies** - Shadowcasting produces different visibility results depending on the

   observer's position relative to the same geometry.
   [Issue #218](https://github.com/ondras/rot.js/issues/218)

1. **Shadowcasting performance for real-time** - `_getCircle` allocates memory on every call. For
...
1. **No input system** - Only provides a `KEYS` constant with deprecated `keyCode` mappings. No
...
1. **"Feature-complete" / low maintenance** - The author considers it feature-complete. Issues and
...
1. **Strictly cell-based display** - No support for smooth scrolling, sub-cell positioning,
...
1. **String-key data model** - Display data is stored in a flat object keyed by `"x,y"` strings.

   This works but is not memory-efficient for large maps and involves string allocation/parsing
   overhead.

1. **WebGL backend has rough edges** - TileGL sets `window.gl` as a global (debugging artifact left

   in), uses `alert()` for WebGL initialization failures, and requires WebGL2 with no fallback.

1. **API uses deprecated web standards** - The `KEYS` constant uses `keyCode` values, which are
