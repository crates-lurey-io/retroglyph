...
2. **Author lineage** - Created by jice-nospam, who also created libtcod (The Doryen Library), the
...
3. **Single-draw-call GPU rendering** - The entire console is rendered in one GPU draw call. Three
...
4. **GLSL shader detail** - The fragment shader does: (a) map screen pixel to console cell
...
5. **Subcell resolution** - Uses special characters (ASCII 226-232) to achieve 2x resolution. The
...
6. **Trait-based Engine pattern** - Users implement the `Engine` trait with `init()`, `update()`,
...
7. **Console API** - Cell-based: `ascii(x, y, char)`, `fore(x, y, color)`, `back(x, y, color)` for
...
8. **Colored text markup** - `print_color()` uses `#[color_name]` inline markers:
...
9. **Font support** - Expects a PNG spritesheet with 16x16 character grid layout. Auto-detects three
...
10. **WASM support** - Compiles to `wasm32-unknown-unknown` via wasm-pack. The `uni-gl` and
...
11. **Image blitting** - PNG images can be blitted to console backgrounds at 1:1 or 2x subcell
...
12. **Companion crate: doryen-fov** - Separate crate for 2D field-of-view algorithms. Used in
...
13. **Examples and demos** - Ships with 10 examples: basic (walking @), perf test, fonts, unicode,
...
14. **No known games shipped with it** - No notable games or tools found built with doryen-rs. The
...
15. **Development activity** - Created June 2018. Latest release: v1.4.0 (unreleased on crates.io;
