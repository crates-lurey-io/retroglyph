//! wasm32-only DPR-capping, browser-viewport-fill, and pointer-rescaling helpers used by `super::run`
//! on `wasm32`.
...
/// Upper bound on the device pixel ratio used to size the canvas backing store. Present cost is
/// O(pixels), so an uncapped DPR (3 on many phones, 2 on most laptops) quadruples or worse the
/// per-frame rasterize/present work for marginal crispness on a pseudo-graphic UI.
#[cfg(target_arch = "wasm32")]
const MAX_DEVICE_PIXEL_RATIO: f64 = 1.5;

/// The browser viewport's CSS width/height, or `None` if running outside a browser `window`
/// context. Shared by the two physical-size helpers below.
#[cfg(target_arch = "wasm32")]
...
/// different ratio (like our DPR-capped surface size) makes the canvas's CSS size come out
...
/// [`CellGeometry::pixel_to_cell`](crate::geometry::CellGeometry::pixel_to_cell), are expressed in.
///
...
/// south-east of the intended cell, growing with how far `real_dpr` exceeds the cap (2x at DPR
/// 3 against a 1.5 cap). Pure math, kept separate from [`wasm_pointer_scale`] so it's
/// unit-testable without a wasm window (hence `cfg(any(.., test))`: unused on a native non-test
/// build, whose `on_cursor_moved` hardcodes `scale = 1.0` instead of calling this).
#[cfg(any(target_arch = "wasm32", test))]
fn dpr_pointer_scale(real_dpr: f64, capped_dpr: f64) -> f64 {
    (capped_dpr / real_dpr).min(1.0)
}

/// [`dpr_pointer_scale`] using the page's actual `devicePixelRatio` and [`MAX_DEVICE_PIXEL_RATIO`].
/// `1.0` if no browser `window` is available.
#[cfg(target_arch = "wasm32")]
...
/// Re-requests the window's inner size to match the browser viewport on every `resize` event, so
/// the canvas keeps filling the screen instead of staying pinned to its size at first paint.
#[cfg(target_arch = "wasm32")]
