...
// `app_entry!`'s own doc comment (on the macro, not this private module) is what shows up in
// rustdoc; `#[macro_export]` lifts it to the crate root regardless of this module's visibility,
...
/// input queue: an app that reacts to `Event::Resize` rather than re-checking
/// `term.backend().size()` every frame silently keeps its old layout. [`app_entry!`] already
/// calls this on every `wasm_app_resize`; reach for it directly only when driving a
/// `Terminal<TerminalWasm>` by hand instead of through that macro. See retroglyph#684.
///
/// # Examples
///
/// ```
...
/// ```
///
/// [`app_entry!`]: crate::app_entry
pub fn resize_terminal(term: &mut Terminal<TerminalWasm>, width: u16, height: u16) {
    term.resize(width, height);
    Input::push_event(term.backend_mut(), Event::Resize(width, height));
}
...
mod tests {
    ...
    #[test]
    fn resize_terminal_resizes_grid_backend_and_queues_resize_event() {
        // retroglyph#684: `term.resize(..)` alone (or just `Output::resize` on the backend)
        // leaves nothing in the input queue, so an app that reacts to `Event::Resize` rather than
        // re-checking `term.backend().size()` every frame would silently keep its old layout.
        let mut term = Terminal::new(TerminalWasm::new(10, 3));
        resize_terminal(&mut term, 20, 6);

        assert_eq!(term.backend().size(), Size::new(20, 6));
        assert_eq!(term.grid().width(), 20);
        assert_eq!(term.grid().height(), 6);
        assert_eq!(
            term.poll(Duration::ZERO),
            Some(Event::Resize(20, 6)),
            "resize_terminal must queue the matching Event::Resize"
        );
    }
