...
terminal emulator works the same way). It's a wiring template, not a full game -- it plumbs
input/output through this handle-based FFI but calls no per-frame drawing logic of its own (that's
the consumer's job, via their own Rust code holding the `Terminal<TerminalWasm>`). For a complete,
running example with an actual game loop, see the
