...
//! - the generic blocking driver ([`run`]/[`run_with`], and the `Terminal`-taking
//!   [`run_blocking`]/[`run_blocking_with`] they're built on, `std` only), which covers
...
/// Builds a [`Terminal`] over `backend` and drives `app` with [`run_blocking`].
///
...
/// # Errors
///
/// Returns `backend`'s error if it fails to build a [`Terminal`] over itself, or if the automatic
/// `present()` call fails while `app` is running. See [`run_blocking`] for the exact loop
/// behavior.
#[cfg(feature = "std")]
...
/// Builds a [`Terminal`] over `backend` and drives `app` with [`run_blocking_with`], paced by
/// `options`.
///
/// The `options`-taking counterpart to [`run`]; see that function for which backends this suits,
/// and [`RunOptions`] for the available pacing and idle-blocking controls.
///
/// # Errors
///
/// Returns `backend`'s error if it fails to build a [`Terminal`] over itself, or if the automatic
/// `present()` call fails while `app` is running. See [`run_blocking_with`] for the exact loop
/// behavior.
///
/// # Panics
///
/// Panics if `options.target_fps` is `Some(0)`; see [`run_blocking_with`].
#[cfg(feature = "std")]
