//! Benchmarks for `Grid::fill_rect` (the shared body of `Surface::clear`,
//! `Surface::clear_region`, and `Surface::fill_rect`) on a grid tiled with multi-cell spans.
//!
//! retroglyph#1020: `fill_rect` used to call `clear_span_overlap` once per row, with the whole
...
//! pathological input. This benchmark measures `fill_rect` on exactly that grid shape: every
...
//! into `fill_rect`'s baseline cost.
...
/// Registers a `fill_rect` case per grid, at a fixed grid size.
fn bench_fill_rect(c: &mut Criterion, cols: u16, rows: u16) {
    let mut group = c.benchmark_group(format!("fill_rect/{cols}x{rows}"));
    ...
    group.bench_function("no_spans", |b| {
        let mut grid = Grid::new(cols, rows);
        b.iter(|| {
            grid.fill_rect(0, rect, tile);
            black_box(());
        });
    });

    group.bench_function("fully_spanned_2x2", |b| {
        // `fill_rect` overwrites every span it clears with plain tiles, so a grid reused across
...
fn fill_rect(c: &mut Criterion) {
    ...
    bench_fill_rect(c, 80, 24);
    bench_fill_rect(c, 200, 60);
}

criterion_group!(benches, fill_rect);
