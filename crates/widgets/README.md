...
`Scrollbar`, `ProgressBar`, `Modal`, `StatBar`, `Meter`, `Log`, ...) is a builder struct that draws
itself into a `Surface` (an area-relative view over a `Grid`) via `Widget`/`StatefulWidget` and
retains no state of its own -- state that outlives one render call (a selection index, a scroll
offset) lives in `ListState` instead. A handful of things that are genuinely just functions
