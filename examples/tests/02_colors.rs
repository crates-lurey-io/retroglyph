//! Snapshot tests for the `02_colors` example.
//!
...
// `colors` modules, which is exactly what the lint is (correctly, for a
...
fn headless_snapshot() {
    // Static content (the palette never changes frame to frame), so one
    // frame is enough -- unlike an animated example, additional frames
    // would only duplicate identical text.
    insta::assert_snapshot!(support::headless_snapshot::<Colors>(1));
}

#[cfg(all(feature = "software", not(target_arch = "wasm32")))]
#[test]
fn png_snapshot() {
    let png = support::png_snapshot::<Colors>(50, 25, 2);
...
#[test]
fn svg_snapshot() {
    let bin = support::build_crossterm_example("02_colors");
    let raw = support::capture_pty(&bin, b"", 25, 50, "Indexed");
    let svg = support::svg_snapshot(&raw, 25, 50);
    assert!(svg.contains("Indexed"), "SVG output missing expected text");
    support::write_snapshot_file("02_colors.svg", svg.as_bytes());
