...
fn run(input: &[u8], env: &[(&str, &str)]) -> String {
    let bin = support::build_crossterm_example(EXAMPLE);
    let raw = support::capture_pty_with_env(&bin, input, ROWS, COLS, READY_MARKER, env);
